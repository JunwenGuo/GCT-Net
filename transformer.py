...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None):
        ...
        self.d_model_per_head = d_model // num_heads # 64
    ...
    def forward(
        ...
        """
        q = rearrange(self.proj_q(input_q), 'b n (h c) -> b h n c', h=self.num_heads) # [4, 55, 64]
        k = rearrange(self.proj_k(input_k), 'b m (h c) -> b h m c', h=self.num_heads) # [4, 40, 64]
        v = rearrange(self.proj_v(input_v), 'b m (h c) -> b h m c', h=self.num_heads) # [4, 40, 64]

        attention_scores = torch.einsum('bhnc,bhmc->bhnm', q, k) / self.d_model_per_head ** 0.5 # [4, 55, 40]
...
class AttentionLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None):
        ...
        self.attention = MultiHeadAttention(d_model, num_heads, dropout=dropout) # d_model=256, num_heads=4
...
class AttentionOutput(nn.Module):
    def __init__(self, d_model, dropout=None, activation_fn='ReLU'):
        ...
        self.expand = nn.Linear(d_model, d_model * 2) # d_model=256
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        ...
        self.attention = AttentionLayer(d_model, num_heads, dropout=dropout) # d_model=256, num_heads=4
...
class RPEConditionalTransformer(nn.Module):
    def __init__(
        ...
        self.blocks = blocks # ['self', 'cross', 'self', 'cross', 'self', 'cross']
        layers = []
        ...
        self.return_attention_scores = return_attention_scores # False
