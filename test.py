...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    network_infor_list = ["geo_losses", "cla_losses"]
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            loss, geo_loss, cla_loss= match_loss.run(cur_global_step, test_data, logits, ys_ds, e_hat, y_hat)
            info = [geo_loss, cla_loss]
