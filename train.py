...
def train_step(step, optimizer, model, match_loss, data):
    ...
    loss, geo_loss, cla_loss = match_loss.run(step, data, logits, ys_ds, e_hat, y_hat)
    #loss += loss_i
...
def train(model, train_loader, valid_loader, config):
    ...
    if config.resume:
        ...
        logger_train = Logger(os.path.join(config.log_path, 'log_train.txt'), title='gct', resume=True)
        logger_valid = Logger(os.path.join(config.log_path, 'log_valid.txt'), title='gct', resume=True)
    else:
        best_acc = -1
        start_epoch = 0
        logger_train = Logger(os.path.join(config.log_path, 'log_train.txt'), title='gct')
        logger_train.set_names(['Learning Rate'] + ['Geo Loss', 'Classfi Loss']*(config.iter_num+1))
        logger_valid = Logger(os.path.join(config.log_path, 'log_valid.txt'), title='gct')
        logger_valid.set_names(['Valid Acc'] + ['Geo Loss', 'Clasfi Loss'])
    ...
    for step in trange(start_epoch, config.train_iter, ncols=config.tqdm_width):
        ...
        if b_validate:
            va_res, geo_loss, cla_loss = valid(valid_loader, model, step, config)
            logger_valid.append([va_res, geo_loss, cla_loss])
            if va_res > best_acc:
